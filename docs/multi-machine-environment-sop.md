...
| --- | --- | --- | --- |
| 單機母模板／機器映像 | `VMTemplate` 或 PVE VM／LXC image | 已安裝作業系統與工具的單台來源機器，僅供建構環境 | 不可見 |
...
| `users` | 僅指定學生 |
| `campus` | 全體已登入學生 |
| `system` | 系統管理員維護的全平台環境 |

目前系統尚未實作獨立 audience 欄位。在完成前，`quick_practice`／`both` 一經發布即視為 `campus`，發布畫面必須明確警告「全體已登入學生皆可看見並啟動」。
...
| 差異 | 現況 | 完成標準 |
| --- | --- | --- |
| 沒有 audience | 快速練習發布後全體登入者可見 | 支援 private、class、users、campus、system |
| 沒有發布審核 | 教師發布立即生效 | 高風險能力進入管理員審核 |
| 沒有完整下架／封存 | 主要只有 draft、published | 可停止新啟動並保留既有 Session |
| 沒有環境全域並行上限 | 只有每位學生的限制 | 每個環境可設定最大並行 Session |
| 母模板引用保護不足 | 已發布版本可能失去來源 | hard delete 前強制引用檢查與退役流程 |
