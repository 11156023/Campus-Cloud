...
function MachineEditor({ value, edges, onChange, onEdgesChange, pveTemplates, vmImages, lxcImages, sourceNotice, locked = false }) {
  ...
  return <section className={`${styles.card} ${styles.templateMachineWorkspace}`}>
      <div className={styles.machineWorkspaceHeader}>
        <div><h2>{t("CourseTemplateEditorPage.multiMachineEnvTitle")}</h2><p>{t("CourseTemplateEditorPage.multiMachineEnvDesc")}</p></div>
      ...
      {value.length ? <>
        <div className={styles.topologyHelp}><MIcon name="account_tree" size={17} /><span>{t("CourseTemplateEditorPage.topologyHelpText")}</span></div>
...
export default function CourseTemplateEditorPage() {
    ...
    TemplatesService.list()
      .then((result) => {
        ...
        if (ready.length) setSourceNotice("");
          ...
          setSourceNotice(t("CourseTemplateEditorPage.noTemplatesNotice"));
        }
      })
      .catch((reason) => {
  ...
  return <div className={styles.page}>
    ...
    <PageHeader title={isNew ? t("CourseTemplateEditorPage.createTemplateTitle") : template.name} subtitle={isNew ? t("CourseTemplateEditorPage.createTemplateSubtitle") : `v${template.version} · ${template.updatedAt}`}><div className={styles.pageActions}><button type="button" className={styles.btnSecondary} onClick={() => navigate(returnTo ?? "/course-template-management")}>{t("CourseTemplateEditorPage.backBtn")}</button>{locked ? <button type="button" className={styles.btnPrimary} disabled={saving} onClick={newVersion}><MIcon name="content_copy" size={16} />{t("CourseTemplateEditorPage.createNewVersionBtn")}</button> : <><button type="button" className={styles.btnSecondary} disabled={isNew || saving || !template.name.trim() || missingAudienceClass || template.nodes.length === 0 || template.nodes.length > 3 || invalidTopology} onClick={publish}><MIcon name="lock" size={16} />{t("CourseTemplateEditorPage.saveAndPublishLockBtn")}</button><button type="button" className={styles.btnPrimary} disabled={saving || !template.name.trim() || missingAudienceClass || template.nodes.length === 0 || template.nodes.length > 3 || invalidTopology} onClick={save}><MIcon name="save" size={16} />{saving ? t("CourseTemplateEditorPage.savingEllipsis") : t("CourseTemplateEditorPage.saveDraftBtn")}</button></>}</div></PageHeader>
