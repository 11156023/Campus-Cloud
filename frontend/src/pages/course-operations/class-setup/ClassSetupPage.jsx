...
function SummaryLine({ done, label, value }) {
  return (
    <div className={done ? styles.summaryDone : styles.summaryPending}>
      <span>
        <MIcon name={done ? "check" : "radio_button_unchecked"} size={17} />
      </span>
      <div>
        <strong>{label}</strong>
        <small>{value}</small>
...
export default function ClassSetupPage() {
  ...
  const selectedTemplate = templates.find(
    (template) => String(template.versionId) === String(templateId),
  );
  const completed = [
    Boolean(item),
    Boolean(item?.students.length),
    Boolean(item?.course_environment && item?.nodes.length),
    weeks.some((week) => String(week.title ?? "").trim()),
  ];
  ...
  function applyClass(result) {
    ...
    if (normalized.course_version_id)
      setTemplateId(String(normalized.course_version_id));
    ...
    CourseEnvironmentsService.listPublished()
      .then((rows) => {
        ...
        if (created && rows.some((row) => String(row.id) === String(created))) {
          setTemplateId(
            String(
              rows.find((row) => String(row.id) === String(created)).versionId,
            ),
          );
      ...
      .catch(
        (reason) =>
          active && setMessage(reason?.message ?? "無法讀取已發布教學環境"),
      );
    return () => {
      active = false;
    };
  ...
  useEffect(() => {
    ...
    TeachingClassesService.get(classId)
      ...
      .catch(
        (reason) => active && setMessage(reason?.message ?? "無法讀取班級草稿"),
      )
  ...
  useEffect(() => {
    if (step !== 5 || !classId || !item?.students.length || !item?.nodes.length)
      return undefined;
    let active = true;
    setCapacity(null);
  ...
  function go(nextStep) {
    setParams(
      classId
        ? { classId, step: String(nextStep) }
        : { step: String(nextStep) },
    );
    window.scrollTo({ top: 0, behavior: "smooth" });
  }
  function createTemplate() {
    navigate(templateBuilderPath(classId));
  }
  function pauseSetup() {
    navigate("/class-management", {
      state: {
        message: `「${item?.name ?? "班級"}」已保存為草稿，可隨時回來繼續。`,
      },
    });
  }
  ...
  async function saveBasic() {
    ...
    const saved = classId
      ? await TeachingClassesService.update(classId, payload)
  ...
  async function saveStudents() {
    ...
    if (!item?.students.length && !parsed.length) {
      setMessage("請至少加入一位學生。");
      return false;
    }
    if (parsed.length) {
      ...
      if (result.not_found?.length)
        setMessage(
          `已加入 ${result.added} 位；找不到：${result.not_found.join("、")}`,
        );
    }
    return true;
  }
  ...
  async function saveTasks() {
    applyClass(
      await TeachingClassesService.replaceWeeks(classId, weekPayload(weeks)),
    );
    return true;
  }
  ...
  async function next() {
    ...
    try {
      ...
      setMessage(reason?.message ?? "儲存失敗，請稍後再試。");
    } finally {
      setBusy(false);
    }
  }
  ...
  async function provision() {
    ...
    try {
      ...
      navigate(`/class-management/${result.id}`, {
        replace: true,
        state: { message: "班級設定已完成並送出建機審核。" },
      });
    } catch (reason) {
      setMessage(reason?.message ?? "送出建機失敗");
    } finally {
      setBusy(false);
    }
  }
  ...
  const taskCount = useMemo(
    () => weeks.filter((week) => String(week.title ?? "").trim()).length,
    [weeks],
  );
  ...
  return (
    <div className={styles.page}>
      ...
      <PageHeader
        eyebrow="建立班級"
        title={item?.name || "建立完整課程班級"}
        subtitle="依序完成課表、學生、環境與每週任務；每一步都會保存到正式班級。"
      >
        <span className={styles.saveState}>
          <MIcon name="cloud_done" size={16} />
          {classId ? "班級草稿已建立" : "第一步後開始自動保存"}
        </span>
      </PageHeader>

      <nav className={styles.stepper} aria-label="建立班級流程">
        {STEPS.map(([key, label, hint], index) => {
          ...
          return (
            <button
              type="button"
              key={key}
              disabled={!classId && number > 1}
              className={`${step === number ? styles.stepActive : ""} ${done ? styles.stepDone : ""}`}
              onClick={() => number <= step && go(number)}
            >
              <span>{done ? <MIcon name="check" size={15} /> : number}</span>
              <div>
                <strong>{label}</strong>
                <small>{hint}</small>
              </div>
            ...
            <div className={styles.formGrid}>
              <label className={styles.full}>
                ...
                <input
                  ...
                  placeholder="例如：Linux Web 實務｜115-1"
                  autoFocus
                />
              </label>
              <label className={styles.full}>
                ...
                <input
                  ...
                  placeholder="例如：電腦教室 A（會顯示在學生今日課表）"
                />
              </label>
              <label>
                <span>開始日期</span>
          ...
          <section className={styles.card}>
            <div className={styles.sectionHeader}>
              ...
              <div>
                ...
                <p>
                  貼上
                  Email；可以使用換行、逗號或分號分隔。找不到的帳號會單獨回報。
                </p>
              </div>
            </div>
            <div className={styles.studentLayout}>
              <label>
                ...
                <textarea
                  ...
                  placeholder={"student01@example.edu\nstudent02@example.edu"}
                  autoFocus
                />
                <small>
                  準備加入 {parseStudentEmails(emails).length} 個 Email
                </small>
              </label>
              <aside>
                <strong>目前班級學生</strong>
                <span>
                ...
                {item?.students.slice(0, 5).map((student) => (
                  <em key={student.id}>{student.full_name || student.email}</em>
                ))}
              </aside>
            </div>
          </section>
          ...
          <section className={styles.card}>
            <div className={styles.sectionHeader}>
              ...
              <div>
                <h2>每位學生的教學環境</h2>
                <p>
                  選擇已發布的固定版本。送出建機後，每位學生會取得相同機器組合。
                </p>
              </div>
            </div>

            <div className={styles.templatePauseCard}>
              <span className={styles.templatePauseIcon}>
                <MIcon name="bookmark_added" size={22} />
              </span>
              <div>
                <strong>還沒有合適的上課模板？</strong>
                <p>
                  班級資料與學生名單已保存為草稿。可以先去建立模板；完成並發布後，系統會自動回到這一步並替你選取。
                </p>
                <small>
                  <MIcon name="cloud_done" size={14} />
                  離開不會遺失前兩步資料
                </small>
              </div>
              ...
              <div className={styles.environmentGrid}>
                {templates.map((template) => (
                  <button
                    ...
                    className={
                      String(template.versionId) === String(templateId)
                        ? styles.environmentSelected
                        : ""
                    }
                    onClick={() => setTemplateId(String(template.versionId))}
                  >
                    <span>
                      <MIcon name="account_tree" size={20} />
                    </span>
                    <div>
                      <strong>{template.name}</strong>
                      <p>{template.description || "沒有補充說明"}</p>
                      <small>
                        每位 {template.nodes.length} 台 · v{template.version} ·
                        已發布
                      </small>
                    </div>
                    <em>
            ...
            {selectedTemplate && (
              <div className={styles.environmentSummary}>
                <strong>
                  全班預估{" "}
                  {Number(item?.students.length ?? 0) *
                    selectedTemplate.nodes.length}{" "}
                  台機器
                </strong>
                <span>
                  {item?.students.length ?? 0} 位學生 × 每位{" "}
                  {selectedTemplate.nodes.length} 台
                </span>
              </div>
            )}
          </section>
        ...
        {step === 4 && (
          <section className={styles.card}>
            <div className={styles.sectionHeader}>
              <span>4</span>
              <div>
                <h2>安排每週任務與 Checkpoint</h2>
                <p>
                  課次日期由固定課表產生。先填每週主題或
                  checkpoint，教材檔案之後仍可在班級內補充。
                </p>
              </div>
              <em>
                {taskCount}/{weeks.length} 週已設定
              </em>
            </div>
            <div className={styles.weekList}>
              {weeks.map((week, index) => (
                <label key={week.id ?? week.session_date}>
                  <span>
                    <strong>第 {week.week_number ?? index + 1} 週</strong>
                    <small>{week.session_date}</small>
                  </span>
                  <input
                    value={week.title ?? ""}
                    onChange={(event) =>
                    ...
                    placeholder="例如：完成 API 與資料庫連線 checkpoint"
                  />
                </label>
              ))}
            </div>
          ...
          <section className={styles.reviewLayout}>
            <div className={styles.card}>
              ...
              <div className={styles.summaryList}>
                <SummaryLine
                  ...
                  value={
                    item
                      ? `${item.start_date} 至 ${item.end_date} · 每週${["一", "二", "三", "四", "五", "六", "日"][item.weekday]} ${String(item.start_time).slice(0, 5)}`
                      : "尚未建立"
                  }
                />
                <SummaryLine
                  done={Boolean(item?.students.length)}
                  label="學生名單"
                  value={`${item?.students.length ?? 0} 位學生`}
                />
                <SummaryLine
                  done={Boolean(item?.course_environment && item?.nodes.length)}
                  label="教學環境"
                  value={
                    item?.course_environment
                      ? `${item.course_environment.name} · 每位 ${item.nodes.length} 台`
                      : "尚未選擇"
                  }
                />
                <SummaryLine
                  ...
                  label="每週任務"
                  value={`${taskCount}/${weeks.length} 週已設定；可在開課後繼續補充`}
                />
              </div>
            </div>
            <aside className={styles.capacityCard}>
              <span className={styles.capacityIcon}>
                <MIcon
                  name={capacity?.ready ? "check" : "hourglass_top"}
                  size={24}
                />
              </span>
              <h2>
              ...
              {capacity?.ready ? (
                <>
                  <p>
                    全班將建立 {capacity.machine_count} 台機器，預留{" "}
                    {capacity.ip_count} 個 IP。
                  </p>
                  <dl>
                    <div>
                      <dt>CPU</dt>
                      <dd>{capacity.cpu_cores}</dd>
                    </div>
                    <div>
                      <dt>RAM</dt>
                      <dd>{Math.round(capacity.memory_mb / 1024)} GB</dd>
                    </div>
                    <div>
                      <dt>Disk</dt>
                      <dd>{capacity.disk_gb} GB</dd>
                    </div>
                  </dl>
                </>
              ) : (
                <p>
                  {capacity?.issues?.join("；") ??
                    "正在依學生人數、環境規格與課程期間確認容量。"}
                </p>
              )}
              <button
                type="button"
                className={styles.btnPrimary}
                disabled={!capacity?.ready || busy}
                onClick={provision}
              >
                <MIcon name="rocket_launch" size={17} />
                {busy ? "送出中…" : "完成設定並送出建機"}
              </button>
              <button
                type="button"
                className={styles.btnSecondary}
                onClick={() => navigate(`/class-management/${classId}`)}
              >
                先保存草稿，稍後送出
              </button>
            </aside>
      ...
      {step < 5 && (
        <footer className={styles.footer}>
          ...
          {step === 3 && !templateId ? (
            <em className={styles.footerHint}>
